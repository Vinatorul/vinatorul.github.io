...
Здесь рассматриваются две различные квадратичные сортировки и примеры реализации.
<!--more-->
...
Сформулируем эту идею более формально. Пусть задан массив целых чисел $$A = [a_1, a_2, a_3, \dots, a_i, \dots, a_n]$$ и пусть $$a_i$$~--- минимум в этом массиве, тогда поменяем местами $$a_1$$ и $$a_i$$, получим массив $$A = [a_i, a_2, a_3, \dots, a_1, \dots, a_n]$$. Рассмотрим теперь подмассив $$[a_2, a_3, \dots, a_1, \dots, a_n]$$, найдём в нём минимум и снова поменяем местами с первым элементом.
...
Минимальный элемент в нём: 1, поменяем её с первым элментом массива местами, получаем $$A = [1, 5, 3, 7, 8, 6, 9]$$.

Теперь наёдём минимальный элемент в подмассиве $$[5, 3, 7, 8, 6, 9]$$, это будет число 3. Поменяем его с первым элементом местами, получаем $$A = [1, 3, 5, 7, 8, 6, 9]$$.

Продолжая так далее получим $$A = [1, 3, 5, 6, 7, 8, 9]$$.
...
// Требуется 11 стандарт для такого способа обхода массива
for (auto &i : A)
